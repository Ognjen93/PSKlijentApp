...
public class PregledKarataController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        out.println(ProtocolMessages.KARTE.getMessage());
...
//                  Karta(Integer id,Integer brojReda,Integer brojSjedista,Date termin,Integer idScene)
        ...
        out.println(ProtocolMessages.SJEDISTA.getMessage()+scenaZaPrikaz.getIdScene());
...
//                        +resultSet.getString("idScene")+ProtocolMessages.LINE_SEPARATOR.getMessage();
        ...
        if (sjedisteList.isEmpty()) {
            for (int i = 0; i < RED; i++) {
                for (int j = 0; j < KOLONA; j++) {
                    out.println(ProtocolMessages.DODAVANJE_SJEDISTA.getMessage()+scenaZaPrikaz.getIdScene()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+(i * KOLONA + j));
                    if(in.readLine().startsWith(ProtocolMessages.DODAVANJE_SJEDISTA_OK.getMessage())){
        ...
        comboRezervacije.getItems().removeAll(comboRezervacije.getItems());
            ...
            out.println(ProtocolMessages.REZERVACIJE.getMessage()+strDate+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+scenaZaPrikaz.getIdScene());
            response=null;
            try {
...
//                      Rezervacija(Integer id, String ime, Date termin, Integer idScene)
    ...
    private void postavi(){
        try {
            ...
            DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
            String strDate = dateFormat.format(terminPredstave);
            out.println(ProtocolMessages.SJEDISTA_RS.getMessage() + strDate + ProtocolMessages.MESSAGE_SEPARATOR.getMessage() + scenaZaPrikaz.getIdScene());
            String response = in.readLine();
            ...
            for (int i = 0; i < RED; i++) {
                for (int j = 0; j < KOLONA; j++) {
                    ...
                    buttonMatrix[i][j].setOnMouseClicked(e -> {
                        ...
                        out.println(ProtocolMessages.SJEDISTA_RS.getMessage() + strDate1 + ProtocolMessages.MESSAGE_SEPARATOR.getMessage() + scenaZaPrikaz.getIdScene());
...
//            Rezervacija(Integer id, String ime, Date termin, Integer idScene)
            ...
            out.println(ProtocolMessages.OBRISI_REZERVACIJU.getMessage()+rez.getId()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+rez.getIme()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+
                    strDate1+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+rez.getIdScene());
            if(in.readLine().startsWith(ProtocolMessages.OBRISI_REZERVACIJU_OK.getMessage())){
            ...
            if (!comboRezervacije.getSelectionModel().isEmpty() && !rezervisanaSjedista.isEmpty()) {
                ...
                if (result.get() == odustani) {
                    ...
                    rezervisanaSjedista.forEach(e -> {
                        ...
                        out.println(ProtocolMessages.DODAJ_KARTU.getMessage() + k.getId() + ProtocolMessages.MESSAGE_SEPARATOR.getMessage() + k.getBrojReda() + ProtocolMessages.MESSAGE_SEPARATOR.getMessage() + k.getBrojSjedista() +
                                ProtocolMessages.MESSAGE_SEPARATOR.getMessage() + strDate1 + ProtocolMessages.MESSAGE_SEPARATOR.getMessage() + k.getIdScene());
                        boolean ress = false;
                        try {
                            ress = in.readLine().startsWith(ProtocolMessages.DODAJ_KARTU_OK.getMessage());
    ...
    private void buttonStornirajProdaju() {
        ...
        if (!comboBoxKarte.getSelectionModel().isEmpty()) {
            out.println(ProtocolMessages.OBRISI_KARTU.getMessage()+comboBoxKarte.getSelectionModel().getSelectedItem().getId());
            if(in.readLine().startsWith(ProtocolMessages.OBRISI_KARTU_OK.getMessage())){
...
//            RezervisanoSjediste(Integer idScene,Integer brojSjedista,Integer idRezervacije,Date termin)
            ...
            out.println(ProtocolMessages.OBRISI_REZERVISANO_SJEDISTE.getMessage()+e.getIdScene()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+e.getBrojSjedista()+
                    ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+e.getIdRezervacije()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+strDate1);
            boolean ooo=false;
            try{
                ooo=in.readLine().startsWith(ProtocolMessages.OBRISI_REZERVISANO_SJEDISTE_OK.getMessage());
...
//            RezervisanoSjedisteDAO.obrisiRezervisanoSjediste(e);
        ...
        out.println(ProtocolMessages.OBRISI_REZERVACIJU.getMessage()+rezervacija.getId()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+rezervacija.getIme()+ProtocolMessages.MESSAGE_SEPARATOR.getMessage()+
